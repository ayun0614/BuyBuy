    ...
    if (distance < 0) {
      ...
      <br> <br> <br><br>
       <div class="load-more-container">
            <button class="load-more btn btn-primary btn-lg" style="margin: 0 auto;">더보기</button>
        </div>
      <br>
   </div>
   <script>
   // 페이지 로드 시 처음에 보여지는 상품 개수
   var visibleCount = 8;

   $(document).ready(function () {
       $('.card-ui').slice(0, visibleCount).show();

       $('.load-more').click(function () {
           // 추가로 보여질 상품 개수
           var increment = 4;

           // 다음에 보여질 상품 개수만큼 숨겨진 상품을 나타냄
           $('.card-ui:hidden').slice(0, increment).fadeIn();

           // 현재 보여지는 상품 개수 업데이트
           visibleCount += increment;

           // 더 이상 숨겨진 상품이 없을 경우 더보기 버튼 숨김
